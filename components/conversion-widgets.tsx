...
import type { GlobalUiData } from "@/lib/sanity/types";

type ConversionWidgetsProps = {
  globalUi: GlobalUiData;
};

export function ConversionWidgets({ globalUi }: ConversionWidgetsProps) {
  return (
    <>
      {globalUi.showFloatingCredit !== false && globalUi.floatingCredit?.url ? (
        <Link
          className="floating-credit"
          href={globalUi.floatingCredit.url}
          target="_blank"
          rel="noopener noreferrer"
          aria-label={globalUi.floatingCredit.label || "Designed by Sintyz"}
        >
          ...
          <strong>{globalUi.floatingCredit.label}</strong>
        </Link>
      ) : null}

      {globalUi.showBackToTop !== false ? (
        <Link className="back-to-top" href="#home" aria-label="Back to top">
          ↑
        </Link>
      ) : null}
    </>
